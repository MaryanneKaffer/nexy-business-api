...
export default function LoadingModal({ onLoad }: { onLoad: any }) {
    ...
    return (
        <div className="h-full w-full absolute z-100 bg-default/85 top-0 left-0 flex items-center justify-center">
            <div className={`flex flex-col bg-white dark:bg-black items-center transition-all duration-700 sm:p-5 p-3 rounded-lg ${loaded ? "sm:h-[300px] h-[220px] sm:w-[450px] w-[80dvw]" : "size-[200px]"}`}>
                <Button isLoading size="lg" className={`h-full flex-col bg-transparent transition-all duration-700 ${loaded === false ? "opacity-100" : "opacity-0 size-0"}`}>
                    {loadState}
                </Button>
                <div className={`flex flex-col transition-all duration-700 size-full items-center ${loaded ? "opacity-100" : "opacity-0 size-0"}`}>
