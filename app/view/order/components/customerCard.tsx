...
export default function CustomerCard({ data }: { data: Order }) {
    ...
    return (
        <>
            ...
            <Card className="h-fit sm:p-3 p-1 group hover:scale-102 w-full" radius="sm">
                <CardBody >
                    <div className="flex flex-col gap-3 h-full">
                        <span className="flex items-center gap-3">
                            <IoPerson className="my-auto lg:size-[60px] md:size-[45px] size-[35]" />
                            <span className="flex flex-col leading-tight">
                                <p className="sm:text-xl text-md">{data.customer.corporateName}</p>
                            ...
                            <span className="absolute sm:hidden sm:right-[3dvw] right-[1dvw]"><ViewButton id={data.customer.id} type={"customer"} /></span>
                        </span>
                        {data.customer.id > 0 && <span className="grid grid-cols-2 text-md leading-tight gap-x-3">
                            {Object.entries(data.customer).filter(([key, _]) => !excludeCustomerKeys.includes(key)).map(([key, value]) => (
                                <p key={key} className="sm:text-md text-[10px] sm:mt-2 mt-0.5">{key.toUpperCase()}: {String(value)}</p>
                            ))}
                        </span>}
                    </div>
                </CardBody>
